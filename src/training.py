...
class Trainer:
    ...
    def print_status(self):
        ...
        latest_discrete_actor_loss = 0.0
        latest_continuous_actor_loss = 0.0
        avg_actor_loss = 0.0
    ...
    def save(self, directory: str):
        ...
        self.state_encoder.save(f"{directory}/state_encoder.pth")
        ...
        data = {
            ...
            "discrete_actor_loss": self.discrete_actor_losses,
            "continuous_actor_loss": self.continuous_actor_losses,
    ...
    def load(self, directory: str):
        ...
        self.state_encoder.load(f"{directory}/state_encoder.pth")
    ...
    def run_episode(self):
        ...
        while True:
            ...
            # Feedforward to discrete actor
            discrete_action_distribution = self.discrete_actor(encoded_state)
            discrete_action = discrete_action_distribution.sample()
            discrete_log_prob = discrete_action_distribution.log_prob(
                discrete_action).detach().numpy()
            discrete_action = discrete_action.detach().numpy()

            # Feedforward to continuous actor
            continuous_action_distribution = self.continuous_actor(encoded_state)
            continuous_action = continuous_action_distribution.sample()
            continuous_log_prob = continuous_action_distribution.log_prob(
                continuous_action).detach().numpy()
            continuous_action = discrete_action.detach().numpy()
            ...
            log_prob = {
                'discrete': discrete_log_prob,
                'continuous': continuous_log_prob,
            }
    ...
    def calculate_normalized_advantage(
        ...
        V = self.critic(features).detach().squeeze()
    ...
    def training_step(
        ...
        # Discrete
        current_discrete_distributions = self.discrete_actor(features)
        current_discrete_log_probabilities = (
            current_discrete_distributions.log_prob(
            actions['discrete']))

        # Continuous
        current_continuous_distributions = self.continuous_actor(features)
        current_continuous_log_probabilities = (
            current_continuous_distributions.log_prob(actions['continuous']))
        ...
        discrete_ratio = torch.exp(current_discrete_log_probabilities -
                                   log_probabilities['discrete'])
        continuous_ratio = torch.exp(current_continuous_log_probabilities -
                                     log_probabilities['continuous'])

        # Now we calculate the actor loss for this step
        discrete_actor_loss = -torch.min(
            discrete_ratio * normalized_advantage,
            torch.clamp(discrete_ratio, 1 - self.ε, 1 + self.ε) *
            normalized_advantage,
        ).mean()

        continuous_actor_loss = -torch.min(
            continuous_ratio * normalized_advantage,
            torch.clamp(continuous_ratio, 1 - self.ε, 1 + self.ε) *
            normalized_advantage,
        ).mean()
        ...
        return (discrete_actor_loss.item(), continuous_actor_loss,
                critic_loss.item())
    ...
    def train(self):
        ...
        while self.current_timestep <= self.timesteps:
            ...
            for c in range(self.training_cycles_per_batch):
                ...
                discrete_actor_loss, continuous_actor_loss, critic_loss = (
                    self.training_step(
                    observations,
                    actions,
                    log_probabilities,
                ...
                self.discrete_actor_losses.append(discrete_actor_loss)
                self.continuous_actor_losses.append(continuous_actor_loss)
